...
long   landr(long, long, long, long, double, double, long, double,
	     double *, double *, double *, char out[128]);
...
void las1(char *in1, char *in2, char *out1, char *out2) 
{
   float t0, exetime;
   ...
   long hcount;
   char title[73], name[41], v[6], line[256];
   FILE *fp_in1, *fp_in2;
...
/*     out2 = "lav1"; */
   ...
   /* read data */
    ...
    title[73] = '\0';
    fgets( line, 255, fp_in1); /* skip first line of comment */
    ...
    fprintf(fp_out1," ...... LANSO EXECUTION TIME=%10.2lE\n",exetime);
    fprintf(fp_out1," ...... \n");
    ...
    if (vectors) {
	  ...
	  id += n;
       ...
       fprintf(fp_out1,"\n");
       fprintf(fp_out1," ...... LASVD EXECUTION TIME=%10lf\n",exetime);
       fprintf(fp_out1," ...... \n");
	  ...
	  if (bnd[i] > kappa * fabs(ritz[i])) break;
       ...
       fprintf(fp_out1," ...... NO. MULT. BY TRANSPOSE(A) =%10ld\n", hcount);
       fprintf(fp_out1,"\n");
       fprintf(fp_out1," ...... LASVD EXECUTION TIME=%10.2lE\n", exetime);
       fprintf(fp_out1," ...... \n");
       fprintf(fp_out1," ......         NSIG = %4ld\n", nsig);
       fprintf(fp_out1," ...... \n");
...
void   ritvec(long, double, double *, double *, double *, double *,
              double *, double *, char outfile[128]);
...
long landr(long n, long lanmax, long maxprs, long nnzero, double endl,
	   ...
	   double *bnd, double *r, char out2[128])

{
   ...
   if (vectors) {
      ...
      ritvec(n, kappa, ritz, bnd, wptr[6], wptr[9], wptr[4], wptr[5], out2);
   }
...

{
   ...
   if(strstr( outfile, "Normal")) {
     doNormal=1;
     if(!(inK = fopen("Ki.tmp", "r"))) {
       printf("Cannot read file \"Ki.tmp\" in LAS1::RITVEC(). Exit.");
       exit(1);
     }
   }     
   ...
   sprintf( out2fmt, "%s-FORMATTED.txt", outfile);

   out = fopen( out2fmt,"w");
   if(!out) { printf("Error opening file in RITVEC()\n"); exit(1); }

   /* on return w1 contains eigenvalues in ascending order and s 
    * contains the corresponding eigenvectors */

   fprintf(out, "# %ld %ld %lg\n", n,js,kappa);
   fprintf(out, "# \n");
   ...
   for (k = 0; k < js; k++) {
	 ...
	 fprintf(out, "EIGENVALUE = %lg\n", fabs(ritz[k]));
 	 fprintf(out, "EIGENVECTOR =\n");
/*  	 fprintf(out, "EIGENVALUE = Look into LCZ-EigVals file\nEIGENVECTOR =\n"); */
         for (i = 0; i < n; i++) {
	   if(doNormal) {
	     if( fscanf(inK, "%ld %ld", &idx, &K)==2)
	       fprintf(out, "%5ld      %+-16g\n",i,(K==0)?w1[i]:w1[i]/sqrt(K));
	   } else {
	     fprintf(out, "%5ld      %+-16g\n",i,w1[i]);
	   }
	 }
